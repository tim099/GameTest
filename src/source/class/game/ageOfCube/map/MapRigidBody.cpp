...
bool MapRigidBody::MapRigidBody::check_collision(physic::RigidBody* b){
				...
				if(map->get_cube_type(i,j,k)!=Cube::cubeNull){
					...
					if(delx>=0&&dely>=0&&delz>=0){
						if(sqrt((b->radius-delx)*(b->radius-delx)+
							...
							if((prev_pos.x>i&&b->vel.x<0&&map->get_cube_type(i+1,j,k)==Cube::cubeNull)||
							   (prev_pos.x<i&&b->vel.x>0&&map->get_cube_type(i-1,j,k)==Cube::cubeNull)){//pos.y==j&&pos.z==k
								c_x=true;
							}
							if((prev_pos.y>j&&b->vel.y<0&&map->get_cube_type(i,j+1,k)==Cube::cubeNull)||
									(prev_pos.y<j&&b->vel.y>0&&map->get_cube_type(i,j-1,k)==Cube::cubeNull)){//pos.x==i&&pos.z==k
								c_y=true;
							}
							if((prev_pos.z>k&&b->vel.z<0&&map->get_cube_type(i,j,k+1)==Cube::cubeNull)||
									(prev_pos.z<k&&b->vel.z>0&&map->get_cube_type(i,j,k-1)==Cube::cubeNull)){//pos.x==i&&pos.y==j
								c_z=true;
							}
						}
					}
	...
	if(handle_stuck(b))collided=true;
	...
	for(int i=s.x;i<=e.x;i++){
		for(int j=s.y;j<=e.y;j++){
			for(int k=s.z;k<=e.z;k++){
				if(map->get_cube_type(i,j,k)!=Cube::cubeNull){
					if(pos.y==j&&pos.z==k&&
					   fabs(b->pos.x-(i+0.5)*Map::CUBE_SIZE)<=0.5*Map::CUBE_SIZE+b->radius){
						return true;
					}
					if(pos.x==i&&pos.z==k&&
					   fabs(b->pos.y-(j+0.5)*Map::CUBE_SIZE)<=0.5*Map::CUBE_SIZE+b->radius){
						return true;
					}
					if(pos.x==i&&pos.y==j&&
					   fabs(b->pos.z-(k+0.5)*Map::CUBE_SIZE)<=0.5*Map::CUBE_SIZE+b->radius){
						return true;
					}
