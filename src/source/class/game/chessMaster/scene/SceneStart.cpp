...
void SceneStart::AI_move(int player){
	...
	if(!ai->is_searching()&&!ai->search_done){
		...
		if(ai->is_searching()){
			//std::cout<<"SceneStart::AI_move already searching!!"<<std::endl;
		}else{
			//std::cout<<"SceneStart::AI_move search done!!"<<std::endl;
		}
	}
}
...
void SceneStart::player_move_chess(){
